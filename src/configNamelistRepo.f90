module config_namelist_repo
    ...
    character(*), public, parameter :: fmt_namelist = "namelist"
        !! format specifier

    type, public, extends(base_repository_atype) :: config_namelist_repository_type
    ...
    end type config_namelist_repository_type

contains
    !>constructs a `config_namelist_repository_type` instance
    subroutine construct(this, args)
        implicit none
        class(config_namelist_repository_type), intent(inout) :: this
        class(repository_constructor_arguments_type), intent(in) :: args

        type(config_namelist_repository_type) :: config
        namelist /simulation_configulation/ config

        integer(int32) :: unit_number, io_stat
        character(128) :: io_msg

        if (args%get_format() == fmt_namelist) then
            open (newunit=unit_number, file=args%get_filename())
            read (unit_number, nml=simulation_configulation, iostat=io_stat, iomsg=io_msg)
            close (unit_number)
        end if

        select type (this); type is (config_namelist_repository_type)
            this = config
        end select
...
end module config_namelist_repo
